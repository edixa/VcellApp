...
class MainActivity : AppCompatActivity() {
    ...
    fun insertar(view:View) {
        ...
        var cantidad = edtCantidad?.text.toString()
        var entrada = edtEntrada?.text.toString()
        var salida = edtSalida?.text.toString()

        if (codigo.isEmpty() == false && marca.isEmpty() == false && modelo.isEmpty() == false && cantidad.isEmpty() == false && entrada.isEmpty() == false && salida.isEmpty() == false) {
            var inventario=ContentValues()
            inventario.put("codigo", codigo)
            inventario.put("marca", marca)
            inventario.put("modelo", modelo)
            inventario.put("cantidad", cantidad)
    ...
    fun buscar(view:View){
        ...
        if(codigo.isEmpty()==false){
            val fila=baseDatos.rawQuery("select from inventario")
        }
    }
}